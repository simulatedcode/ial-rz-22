...
function getLocation() {
  return SITE_CONFIG.location
}

function getCurrentUTC() {
  const now = new Date()
  return `UTC${now.getTimezoneOffset() <= 0 ? '+' : '-'}${Math.abs(Math.floor(now.getTimezoneOffset() / 60))}`
}
...
export default function Header() {
  ...
  const [utc, setUtc] = useState(() => getCurrentUTC())

  useEffect(() => {
    const interval = setInterval(() => {
      setTime(getCurrentTime())
      setUtc(getCurrentUTC())
    }, 1000)
